...
#-c -b 127.0.0.1 -t Temperatur/# -t Humidity/#
"""
...
q=Queue()
timetrackingdict={'starttime':time.time()}
...
def message_handler(client,msg,topic):
    ...
    #data["time"]=ltime
        ...
        #for key in keys:
            #data[key]=msg[key]
    #else:
    if ((topic + str(int(msg[0:4],16)))not in timetrackingdict):
        timetrackingdict[(topic + str(int(msg[0:4],16)))] = timetrackingdict['starttime']
    tdif = tnow-timetrackingdict[(topic + str(int(msg[0:4],16)))]
    tinterval = tdif/((len(msg)-4)/4)
    for i in range(4,len(msg), 4):
        data=collections.OrderedDict()
        data["time_ms"]=timetrackingdict[(topic + str(int(msg[0:4],16)))]+(tinterval*(i/4))
        data["device_id"]=int(msg[0:4],16)
        data["message"]=int(msg[i:i+4],16)
        data["topic"]=topic
        client.q.put(data)

    timetrackingdict[(topic + str(int(msg[0:4],16)))] = tnow
